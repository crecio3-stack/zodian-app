...
Provider execution is a local script, never imported by the shadow module. It uses the approved `gpt-5.6-terra` editorial experiment configuration with JSON-object output and `max_output_tokens: 500`, plus at most one initial call for each of the four approved scenarios. A retry is permitted only after a technically unusable initial response and is recorded separately. It writes raw responses and a call ledger only to `/tmp/zodian-shadow-natural-reader-writer-canary-v1/`; no database, deployment, publication, routing, scheduler, resolver, or production path is involved.
