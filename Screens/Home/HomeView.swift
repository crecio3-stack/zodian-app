...
struct HomeView: View {
    ...
    @State private var todayLensIsPressing = false
    @State private var todayLensDragOffset: CGFloat = 0
    @State private var todayLensMotionPull: CGFloat = 0
    ...
    private var savedReadsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            savedLensArchiveHint
        }
    }

    private var savedLensArchiveHint: some View {
        HStack(spacing: 14) {
            ZStack {
                Circle()
            ...
            VStack(alignment: .leading, spacing: 3) {
                Text(savedDailyReadings.isEmpty ? "Save a Lens" : "\(savedDailyReadings.count) saved \(savedDailyReadings.count == 1 ? "Lens" : "Lenses")")
                    .font(.system(size: 16, weight: .bold))
                    .foregroundStyle(ZD.Color.textPrimary)

                Text("Personal daily horoscopes you want to revisit stay in Profile for now.")
        ...
        VStack(alignment: .leading, spacing: 18) {
            HStack(alignment: .top, spacing: 14) {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text("Your personalized daily horoscope is ready.")
                        .font(.system(size: 13, weight: .medium))
    ...
    private var dailyReadStatPills: some View {
        VStack(alignment: .trailing, spacing: 7) {
            dailyReadStatPill("\(store.points) pts", icon: "sparkles")
            dailyReadStatPill("\(store.streak) streak", icon: "waveform.path.ecg")
        }
    }

    private func dailyReadStatPill(_ title: String, icon: String) -> some View {
        HStack(spacing: 5) {
        ...
        .background(
            Capsule(style: .continuous)
                .fill(ZD.Color.cardAlt.opacity(0.66))
                .overlay(
                    Capsule(style: .continuous)
    ...
    private var todayLensRevealRitual: some View {
        let progress = todayLensRevealProgress

        return ZStack {
            todayLensOrbitField(progress: progress)

            todayLensPreviewContent(progress: progress)
                .padding(.horizontal, 18)
                .padding(.vertical, 24)
                .frame(maxWidth: .infinity, minHeight: 246, alignment: .leading)
                .blur(radius: reduceMotion ? 0 : 7)
                .opacity(reduceMotion ? 0.88 : (todayLensIsPressing ? 0.16 : 0))
                .scaleEffect(0.97, anchor: .center)

            todayLensFocusedPreview(progress: progress)

            todayLensGlassControl(progress: progress)
                .offset(y: reduceMotion ? 0 : todayLensElasticOffset)
        }
        .frame(maxWidth: .infinity)
        .frame(height: reduceMotion ? 220 : 274)
        .contentShape(RoundedRectangle(cornerRadius: 26, style: .continuous))
        .gesture(reduceMotion ? nil : todayLensGesture)
        .onTapGesture {
    ...
    private var todayLensRevealProgress: CGFloat {
        min(1, abs(todayLensDragOffset) / 92)
    }

    private var todayLensElasticOffset: CGFloat {
        todayLensDragOffset + (todayLensMotionPull * 7)
    }

    private var todayLensMotionAmount: CGFloat {
        min(1, abs(todayLensMotionPull))
    }

    private var todayLensControlHeight: CGFloat {
        todayLensIsPressing ? 86 : 62
    }

    private var todayLensControlWidth: CGFloat? {
        todayLensIsPressing ? nil : 184
    }

    private func todayLensPreviewContent(progress: CGFloat) -> some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("A clearer way to see today")
                .font(.system(size: 11, weight: .bold, design: .rounded))
                .tracking(2)
                .textCase(.uppercase)
                .foregroundStyle(ZD.Color.accent.opacity(0.72 + (progress * 0.18)))

            Text(todayLensPreviewTitle)
                .font(.system(size: 34, weight: .bold, design: .serif))
                .foregroundStyle(ZD.Color.textPrimary)
                .lineSpacing(1)
                .fixedSize(horizontal: false, vertical: true)

            if let previewLine = todayLensPreviewLine {
                Text(previewLine)
                    .font(.system(size: 16, weight: .medium))
                    .foregroundStyle(ZD.Color.textSecondary.opacity(0.78 + (progress * 0.18)))
                    .lineSpacing(4)
                    .fixedSize(horizontal: false, vertical: true)
            }

            Text("Release to reveal")
                .font(.system(size: 12, weight: .bold, design: .rounded))
                .tracking(1.2)
                .textCase(.uppercase)
    ...
    private func todayLensFocusedPreview(progress: CGFloat) -> some View {
        GeometryReader { proxy in
            let lensWidth = todayLensIsPressing ? proxy.size.width : 184
            let lensHeight = todayLensControlHeight
            let lensCenterY = proxy.size.height * 0.50 + todayLensDragOffset

            todayLensPreviewContent(progress: progress)
                .padding(.horizontal, 18)
                .padding(.vertical, 24)
                .frame(maxWidth: .infinity, minHeight: 246, alignment: .leading)
                .opacity(reduceMotion ? 0 : (todayLensIsPressing ? 1 : 0))
                .brightness(0.03 + (progress * 0.03))
                .contrast(1.08)
                .mask {
                    Capsule(style: .continuous)
                        .frame(width: lensWidth, height: lensHeight)
                        .position(x: proxy.size.width * 0.50, y: lensCenterY)
                }
        }
        .allowsHitTesting(false)
    }

    private var todayLensPreviewTitle: String {
        if case .loaded(let ritual) = dailyRitualViewModel.state,
           ritual.hasRealDailyReadContent {
            return ritual.title
        }

        return "Your reveal is ready"
    }

    private var todayLensPreviewLine: String? {
        if case .loaded(let ritual) = dailyRitualViewModel.state,
           ritual.hasRealDailyReadContent {
            return ritual.intro.nilIfBlankForHome ?? ritual.validPullQuote
        }

        return "Tap and hold to bring today into focus."
    }
    ...
    private func todayLensGlassControl(progress: CGFloat) -> some View {
        VStack(spacing: 12) {
            ZStack {
                if !todayLensIsPressing {
                    Text("Tap and hold")
                        .font(.system(size: 13, weight: .bold, design: .rounded))
                        .tracking(0.5)
                ...
                .frame(height: todayLensControlHeight)
                .frame(maxWidth: todayLensIsPressing ? .infinity : todayLensControlWidth)
                .background {
                ...
                .shadow(color: ZD.Color.accent.opacity(todayLensIsPressing ? 0.008 : 0.026), radius: todayLensIsPressing ? 5 : 13, x: 0, y: 7)
                .shadow(color: Color.white.opacity(todayLensIsPressing ? 0.016 : 0.022), radius: todayLensIsPressing ? 5 : 9, x: 0, y: 0)
                .scaleEffect(
                    x: todayLensIsPressing ? 1.025 + (progress * 0.018) + (todayLensMotionAmount * 0.016) : 1,
                    y: todayLensIsPressing ? 1.0 - (todayLensMotionAmount * 0.026) : 1,
                    anchor: .center
                )
                .rotationEffect(.degrees(todayLensIsPressing ? Double(todayLensMotionPull * 0.45) : 0))

            if !todayLensIsPressing {
                Text("Bring today into focus")
                    .font(.system(size: 12, weight: .semibold, design: .rounded))
                    .foregroundStyle(ZD.Color.textSecondary.opacity(0.72))
            }
        }
        .padding(.horizontal, todayLensIsPressing ? 0 : 0)
        .animation(.spring(response: 0.36, dampingFraction: 0.72), value: todayLensIsPressing)
        .animation(.interactiveSpring(response: 0.22, dampingFraction: 0.68, blendDuration: 0.14), value: todayLensDragOffset)
        .animation(.interactiveSpring(response: 0.26, dampingFraction: 0.58, blendDuration: 0.08), value: todayLensMotionPull)
    }
    ...
    private func todayLensGlassBackground(progress: CGFloat) -> some View {
        ...
        if todayLensIsPressing, #available(iOS 26.0, *) {
            shape
                .fill(Color.clear)
            ...
            shape
                ...
                .glassEffect(
                    .clear
                        .tint(Color.white.opacity(0.008))
                        .interactive(),
                    in: shape
                )
        } else {
    ...
    private var todayLensGesture: some Gesture {
        LongPressGesture(minimumDuration: 0.18, maximumDistance: 14)
            ...
            .onChanged { value in
                ...
                case .second(true, let drag?):
                    ...
                    let boundedOffset = max(-104, min(104, drag.translation.height))
                    todayLensDragOffset = boundedOffset
                    todayLensMotionPull = max(-1, min(1, (drag.predictedEndTranslation.height - drag.translation.height) / 190))
                default:
                    break
                }
            }
            .onEnded { value in
                guard !isOpeningDailyRitual else {
                    todayLensReset()
                    return
                }

                let didCompleteHold: Bool
                let didPassThreshold: Bool
                ...
                case .second(true, let drag?):
                    didCompleteHold = true
                    didPassThreshold = abs(drag.translation.height) >= 72
                        || todayLensRevealProgress >= 0.72
                default:
                    didCompleteHold = false
                    didPassThreshold = false
                }

                if didCompleteHold || didPassThreshold {
                    todayLensCompleteReveal()
                } else {
                    todayLensReset()
                }
            }
    ...
    private func todayLensBeginPress() {
        guard !todayLensIsPressing else { return }

        todayLensIsPressing = true
        todayLensMotionPull = 0
        feedbackSoft()
    }

    private func todayLensReset() {
        withAnimation(.spring(response: 0.42, dampingFraction: 0.66)) {
            todayLensIsPressing = false
            todayLensDragOffset = 0
            todayLensMotionPull = 0
        }
    }

    private func todayLensCompleteReveal() {
                ...
                HStack(alignment: .top, spacing: 10) {
                    ...
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Lens complete.")
                            ...
                            Text("Today’s horoscope is saved for the day.")
                            .font(.system(size: 13, weight: .medium))
                            .foregroundStyle(ZD.Color.textSecondary)
                    }
                }
    ...
    private func saveDailyRead(_ ritual: DailyRitualResponse) {
        ...
        do {
            ...
            dailyReadSaveMessage = "Lens saved."
            dailyReadSaveDetailMessage = savedCountAfterSave == 2
                ? "Your saved daily horoscopes are waiting in Profile."
                : "Tomorrow brings a new Lens."
    ...
    private func unsaveDailyRead(_ ritual: DailyRitualResponse) {
        ...
        do {
            try context.save()
            dailyReadSaveMessage = "Removed from Profile."
            dailyReadSaveDetailMessage = nil
    ...
    private var relationshipBridgeSection: some View {
        Button {
            feedbackSoft()
            ...
            HStack(alignment: .center, spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("See who fits your rhythm")
                        .font(.system(size: 20, weight: .bold, design: .serif))
                        .foregroundStyle(ZD.Color.textPrimary)

                    Text("See the patterns between people")
                        .font(.system(size: 14, weight: .medium))
                        .foregroundStyle(ZD.Color.textSecondary.opacity(0.82))
                        .lineSpacing(3)
                        .fixedSize(horizontal: false, vertical: true)
            ...
            .background(
                RoundedRectangle(cornerRadius: 26, style: .continuous)
                    .fill(ZD.Color.card.opacity(0.44))
            )
        }
        .buttonStyle(.plain)
    }
    ...
    private var isDailyReadRevealed: Bool {
        store.todayRevealed || isDailyReadExpanded
    }
    ...
    private var saveToHomeCallout: some View {
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                Text("Save this person so their read is waiting in Worth keeping.")
