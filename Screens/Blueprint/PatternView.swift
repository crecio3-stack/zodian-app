...
struct PatternView: View {
    ...
    private var identityCardContent: IdentityCardContent {
        canonicalProfile?.identityCardContent
            ?? editorialProfile?.cardContent
            ?? presentation.identityCardContent
    }

    private var identityPresentation: IdentityPresentation {
        editorialProfile?.presentation ?? IdentityPresentation.make(from: identityContent)
    }

    private var editorialProfile: IdentityEditorialResolvedProfile? {
        guard let user = store.currentUser else { return nil }
        return IdentityEditorialContentService.shared.profile(
            forWestern: user.westernSignRaw,
            chinese: user.chineseSignRaw
        )
    }

    private var canonicalProfile: IdentityProfile? {
        guard let user = store.currentUser else { return nil }
        return IdentityProfileRepository.shared.profile(
            forWestern: user.westernSignRaw,
            chinese: user.chineseSignRaw
        )
    }
                    ...
                    ScrollViewReader { scrollProxy in
                        ScrollView(.vertical, showsIndicators: false) {
                            VStack(alignment: .leading, spacing: 22) {
                                Group {
                                    if let canonicalProfile {
                                        IdentityCanonicalProfileView(
                                            cardContent: identityCardContent,
                                            profile: canonicalProfile,
                                            heroInstruction: "Long press to share what came into focus",
                                            heroShareAccessibilityLabel: "Share your identity",
                                            onShareHero: shareCurrentIdentity,
                                        ...
                                        IdentityEditorialProfileView(
                                            ...
                                            closeCompany: editorialProfile?.closeCompany
                                                ?? presentation.currentArchetype?.compatibilityNotes,
                                            heroInstruction: "Long press to share what came into focus",
                                            heroShareAccessibilityLabel: "Share your identity",
                                            onShareHero: shareCurrentIdentity,
                                            onSectionExpanded: { section in
...
private extension PatternView {
    func scrollExpandedSection(
        _ section: IdentityEditorialSectionKind,
        proxy: ScrollViewProxy
    ) {
        let scroll = {
            // Without an explicit anchor SwiftUI performs the smallest scroll
            // needed to keep the newly opened section visible. That preserves
            // the reader's place instead of snapping every expansion to top.
        ...
        Button {
            ...
            HStack(alignment: .center, spacing: 14) {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("Shift the Lens to someone you know.")
                        .font(.system(size: 14, weight: .medium))
                        .foregroundStyle(ZD.Color.textSecondary)
                }
