        ...
        ZStack {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 22) {
                    topBar
#if DEBUG || BETA
                    if let diagnostics = viewModel.resolvedLensContent?.metadata.betaDiagnostics {
                        VStack(alignment: .leading, spacing: 4) {
                            Text(diagnostics.approvalState)
                                .font(.caption.bold())
                            Text("\(diagnostics.identity) · \(diagnostics.recognitionPacketID)")
                            Text("\(diagnostics.runtimeVersion) · \(diagnostics.cacheStatus)")
                        }
                        .font(.caption2)
...
#endif
#if DEBUG
                    if let metadata = viewModel.debugFreshnessMetadata {
                    ...
                    NavigationLink {
                        ...
                        Label(
                            "Testing Controls · Two-Stage Editorial Review",
                    ...
                    .buttonStyle(.plain)
                    NavigationLink {
                        TodaysLensStructuredVoicePilotV1ReviewView()
                    } label: {
                        Label(
                            "Testing Controls · Structured Voice Pilot",
                    ...
                    if DailyLensCandidateRuntimeFixture.isEditorialVoiceV2RecognitionEnabled {
                        TodaysLensEditorialVoiceV2RecognitionPreviewControls(
                            reload: {
                                Task { await viewModel.reload(for: store.currentUser) }
                            },
                            diagnostics: viewModel.debugEditorialVoiceV2RecognitionDiagnostics
                        )
                        ...
                        TodaysLensEditorialVoiceV21Amendment1PreviewControls(
                            ...
                            diagnostics: viewModel.debugEditorialVoiceV21Diagnostics
                        )
                        ...
                        TodayLensSpokenVoiceV1FixtureControls(
                            ...
                            diagnostics: viewModel.debugLensRoutingDiagnostics
                        )
